...
1. ❌ **容器库** — 不重复 STL 或 Abseil
2. ❌ **异步框架** — 无大型并发原语（不同于 Folly）
3. ❌ **反射系统** — 无运行时类型自省
4. ❌ **"全家桶"工具库** — 克制增长，无功能蔓延
5. ❌ **异常传播** — 错误通过显式返回值处理
6. ❌ **隐式分配** — 所有内存分配必须显式可见
...
1. **统一错误模型** — 模块间一致的错误处理
2. **Parsing 原语** — 更深入的 combinator 支持
3. **小型工具** — string_view-like helpers，零分配
4. **可选日志** — 最小化、编译时可切换
5. **显式内存工具** — unique_ptr、span-like 类型
...
详情见：[positioning.md](../positioning.md)
